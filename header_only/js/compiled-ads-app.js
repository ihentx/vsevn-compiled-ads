...
(function () {
  ...
  function syncDesignViewportUnit() {
    // --fvw = живая 1vw: вёрстка масштабируется браузером сама и остаётся
    // статичной при зуме (vw привязан к вьюпорту, не «растёт» как px).
    // Раньше здесь умножали на эвристически определённый browserZoom и
    // фиксировали px — это и давало рассинхрон/дёргания при масштабировании.
  ...
  function init() {
    // Разовая инициализация раскладки. Реактивный пересчёт на resize/zoom
    // НЕ навешиваем: при vw-статике браузер масштабирует контент сам, без JS.
