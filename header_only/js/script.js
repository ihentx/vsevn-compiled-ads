...
function applyBrowserZoomNeutralizer() {
  // Статический дизайн строится исключительно на vw-единицах (см. var(--fvw, 1vw)
  // в CSS) и SVG-тексте, поэтому масштаб браузера НЕ нейтрализуется через
  // frame.style.zoom. Раньше это давало дёргания (флаки-детект зума + обратная
  // связь по innerWidth) и смещения при прокрутке, а также фактически отключало
  // зум. Теперь функция всегда сбрасывает любые ранее выставленные свойства зума.
...
function updateZoomAwareLines() {
  ...
  // Все «линии»/толщины выражаем через живую vw-единицу (база 1920px:
  // 1 design px = var(--fvw, 1vw) / 19.2). Так они масштабируются ВМЕСТЕ с
  // остальной vw-вёрсткой и остаются неизменными по физическому размеру при
  // зуме (раньше это были фиксированные px, которые при зуме «росли» отдельно
  // от vw-элементов и давали рассинхрон/смещения). Значения зум-инвариантны,
  // поэтому пересчёт на изменение масштаба не нужен.
  const FV = "var(--fvw, 1vw)";
  const dpx = "calc(" + FV + " / 19.2)"; // 1 design px
  const lines = function (k) {
    return "calc(" + FV + " * " + k + " / 19.2)";
  };
  const nextVars = {
    "--page-scale": pageScale.toFixed(6), // безразмерно, только для JS-замеров (init)
    "--dpx": dpx,
    "--ui-half-line": lines(0.5),
    "--ui-hairline": dpx,
    "--ui-control-line": lines(0.5),
    "--ui-control-strong-line": lines(1.5),
    "--ui-checkbox-line": lines(1.25),
    "--ui-checkbox-fill-line": lines(6),
    "--ui-strong-line": lines(2),
    "--zoom-safe-line": dpx, // ≈ 1 design px
    "--zoom-dot-size": lines(2), // dotSize = 2 design px
    "--zoom-dot-radius": dpx, // dotSize / 2 = 1 design px
    "--zoom-dot-edge": lines(1.1), // dotSize * 0.55
    "--zoom-dot-step": lines(5), // dotSize + 3 design px
  };
...
function activateZoomHoverShield() {
  // Отключено: щит наведения активировался на каждый шаг масштабирования и
  // вызывал resetTransientHoverState() (перерисовка иконок навигации, сброс
  // состояний, закрытие меню) — отсюда мигание/смена радиокнопок и всплывающие
  // при зуме подсказки. При vw-статике перерисовка на зум не нужна, поэтому
  // щит больше не активируется.
}
...
function initDateCalendarGlobalHandlers() {
  ...
  // Реактивный пересчёт на resize/zoom отключён: статика построена на vw и
  // масштабируется браузером сама, без JS. Раньше markViewportChanging() на
  // каждый шаг зума пере-применял нейтрализатор и перерисовывал статику —
  // отсюда дёргания и смещения. Открытый календарь по-прежнему позиционируется
  // при прокрутке.
